 ...
 * lib/branding.js. Le logo est lu depuis public/finarent-logo.png au
...
function loadLogo() {
  ...
  try {
    const filePath = path.join(process.cwd(), 'public', 'finarent-logo.png');
    const buf = fs.readFileSync(filePath);
    cachedLogo = { data: `data:image/png;base64,${buf.toString('base64')}`, fmt: 'PNG' };
  } catch {
    cachedLogo = false;
  }
  return cachedLogo;
...
function drawHeader(doc, { title, number, date }) {
  ...
  if (logo && logo.data) {
    try {
      ...
      // Hauteur tirée des proportions réelles du fichier, centrée dans la
      // pastille : une boîte figée déformait le logo à chaque changement.
      const { width, height } = doc.getImageProperties(logo.data);
      const largeur = 44;
      const hauteur = Math.min(12, (largeur * height) / width);
      doc.addImage(logo.data, logo.fmt, 14, 8 + (14 - hauteur) / 2, largeur, hauteur, undefined, 'FAST');
    } catch {
      drawTextLogo(doc);
    }
  } else {
...
function drawIdentityBlocks(doc, { client, extra }) {
  ...
  doc.text(`SIREN : ${c.siren}`, 15, top + 18);
 ...
 * @param {{ bankName: string, holder?: string, iban: string, bic: string }} [banque]
 *   Coordonnées lues par `lireCoordonneesBancaires()` (lib/invoicing/banque.js).
 *   À défaut, le gabarit de company.js — que les routes d'envoi refusent.
 */
export function generateInvoicePDF(invoice, banque = COMPANY_INFO) {
  ...
  doc.setFontSize(8);
  doc.setTextColor(...PRIMARY);
  doc.text(`Virement bancaire : ${banque.bankName}${banque.holder ? ` — ${banque.holder}` : ''}`, 15, footerY + 5);
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  doc.text(`IBAN : ${banque.iban}`, 15, footerY + 9);
  doc.text(`BIC : ${banque.bic}`, 15, footerY + 13);
