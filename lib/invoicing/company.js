...
export function coordonneesBancairesValides() {
  const iban = String(COMPANY_INFO.iban || '').replace(/\s/g, '').toUpperCase();
  const bic = String(COMPANY_INFO.bic || '').replace(/\s/g, '').toUpperCase();

  // Un IBAN français fait 27 caractères ; un gabarit ne contient que des zéros
  // après le code pays et la clé.
