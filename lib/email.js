...
/**
 * Journalise une tentative d'envoi impossible faute de configuration SMTP.
 *
 * Sans ceci, l'echec etait totalement muet : la fonction retournait
 * { sent: false } que personne ne lisait, et rien n'apparaissait nulle part.
 * Une demande pouvait donc etre enregistree sans que quiconque soit prevenu.
 * La trace est visible dans l'admin via EmailLog.
 */
async function tracerEnvoiImpossible(destinataire, sujet) {
  console.error(`[email] SMTP non configure — "${sujet}" non envoye a ${destinataire}`);
  try {
    await prisma.emailLog.create({
      data: {
        type: 'TRANSACTIONAL',
        subject: sujet,
        recipientEmail: destinataire || 'inconnu',
        status: 'FAILED',
        failedAt: new Date(),
        errorMessage: 'SMTP non configuré (SMTP_HOST / SMTP_USER / SMTP_PASS absents)',
        source: 'lib/email.js',
      },
    });
  } catch (e) {
...
export async function sendConfirmationDemande({ to, reference, companyName }) {
  if (!isEmailConfigured()) return tracerEnvoiImpossible(to, `Confirmation de demande ${reference}`);
...
export async function sendAlerteAdmin({ reference, companyName, productType, amount, email }) {
  ...
  if (!isEmailConfigured()) {
    return tracerEnvoiImpossible(adminEmail, `Nouvelle demande ${reference} — ${companyName}`);
  }
...
export async function sendDocumentReceived({ to, fileName, documentType, reference }) {
  if (!isEmailConfigured()) return tracerEnvoiImpossible(to, `Document reçu — ${fileName}`);
  const trans = getTransporter();
  if (!trans) return { sent: false, reason: 'Transporter non configuré' };
...
export async function sendDocumentsMissing({ to, reference, missingDocs = [] }) {
  if (!isEmailConfigured()) return tracerEnvoiImpossible(to, `Pièces manquantes — dossier ${reference}`);
  const trans = getTransporter();
  if (!trans) return { sent: false, reason: 'Transporter non configuré' };
...
export async function sendAffiliateInvite({ to, recipientName, affiliateName, affiliateCode, message }) {
  if (!isEmailConfigured()) return tracerEnvoiImpossible(to, `Invitation de ${affiliateName}`);
  const trans = getTransporter();
  if (!trans) return { sent: false, reason: 'Transporter non configuré' };
