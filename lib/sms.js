...
export async function sendSMS(to, message) {
  ...
  if (!twilioClient) {
    // Twilio absent : ne PAS renvoyer un succes, l'appelant croirait le SMS parti.
    console.error(`[SMS] Twilio non configure — SMS non envoye a ${normalized}`);
    return { success: false, sent: false, reason: 'Twilio non configuré', to: normalized };
  }
...
export const smsTemplates = {
  dossierRecu: (prenom, lienCourt) =>
    `Bonjour ${prenom}, votre demande Finarent est bien reçue. Suivez votre dossier: ${lienCourt}`,
  docManquant: (lienCourt) =>
    `Action requise: un document est à remettre pour votre dossier Finarent. ${lienCourt}`,
  offreDisponible: (mensualite, duree, lienCourt) =>
    `Votre offre Finarent est prête: ${mensualite}€/mois sur ${duree} mois. Consultez: ${lienCourt}`,
  rappelOffre: (lienCourt) =>
    `Rappel: votre offre Finarent expire dans 2 jours. Ne la manquez pas: ${lienCourt}`,
  signatureRequise: (lienCourt) =>
    `Finalisez votre financement: signez votre contrat en ligne. ${lienCourt}`,
  felicitations: (prenom) =>
    `Félicitations ${prenom} ! Votre financement Finarent est accordé. Bienvenue dans notre réseau !`,
};
