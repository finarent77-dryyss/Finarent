...
/**
 * Tous les statuts d'offre que le schéma déclare (enum `OfferStatus`).
 * Sert de liste blanche : ce qui n'y figure pas n'est pas montré au client.
 */
export const STATUTS_OFFRE_CONNUS = Object.freeze([
  'DRAFT', 'SENT', 'VIEWED', 'ACCEPTED', 'REFUSED', 'EXPIRED', 'SIGNED',
]);
...
/**
 * Une offre est-elle montrable au client ?
 *
 * `DRAFT` est exclu : une offre en préparation n'a jamais été transmise et
 * n'existe pas du point de vue du client. Tous les autres statuts connus se
 * montrent, y compris `REFUSED` et `EXPIRED` — masquer une offre périmée
 * donnerait l'impression qu'elle n'a jamais existé, alors que le client doit
 * pouvoir comprendre l'historique de son dossier.
 *
 * Un statut absent ou inconnu ferme, il n'ouvre pas : montrer une offre dont on
 * ignore l'état reviendrait à parier qu'il est inoffensif. Même principe que
 * `offreExpiree`, où une échéance illisible vaut expirée.
 * @param {{ status?: string | null } | null | undefined} offre
 */
export function offreVisibleParLeClient(offre) {
  if (!offre) return false;
  const statut = String(offre.status);
  if (statut === 'DRAFT') return false;
  return STATUTS_OFFRE_CONNUS.includes(statut);
}
