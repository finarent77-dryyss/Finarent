...
const TAG_LEN = 16; // taille du tag d'authentification GCM
const KEY_VERSION = 'v1';

// Un chiffré de ce module ne contient, après « v1: », que de l'alphabet base64
// et le séparateur « : ». Toute autre lettre (accent, tiret, espace…) désigne
// une donnée héritée en clair, quel que soit son début.
const FORME_CHIFFREE = new RegExp(`^${KEY_VERSION}:[A-Za-z0-9+/=:]*$`);

/**
 * Reconnaît la forme exacte d'un chiffré produit par ce module.
 *
 * L'ancienne heuristique se fiait au seul préfixe « v1: » : une note libre
 * commençant par ces trois caractères était prise pour un chiffré et faisait
 * lever une exception au lieu d'être restituée telle quelle.
 *
 * @param {string} payload
 * @returns {{ statut: 'clair' }
 *          | { statut: 'tronque' }
 *          | { statut: 'chiffre', iv: Buffer, authTag: Buffer, ciphertext: Buffer }}
 */
function analyserPayload(payload) {
  if (!FORME_CHIFFREE.test(payload)) return { statut: 'clair' };
  const parts = payload.split(':');
  // Le nombre de segments ne permet pas de trancher : on ne sait pas lequel
  // serait l'IV, donc on ne peut pas écarter un chiffré corrompu. Rendre en
  // clair un secret tronqué serait pire qu'échouer — on échoue.
  if (parts.length !== 4) return { statut: 'tronque' };
  const [, ivB64, tagB64, ctB64] = parts;
  const iv = Buffer.from(ivB64, 'base64');
  const authTag = Buffer.from(tagB64, 'base64');
  // Ce module émet toujours un IV de 12 octets et un tag de 16 : d'autres
  // longueurs ne peuvent pas venir d'ici, c'est donc du clair hérité.
  if (iv.length !== IV_LEN || authTag.length !== TAG_LEN) return { statut: 'clair' };
  if (iv.toString('base64') !== ivB64 || authTag.toString('base64') !== tagB64) {
    return { statut: 'clair' };
  }
  return { statut: 'chiffre', iv, authTag, ciphertext: Buffer.from(ctB64, 'base64') };
}
...
export function decryptString(payload) {
  ...
  const analyse = analyserPayload(payload);
  // Donnée legacy non chiffrée — retournée telle quelle pour migration progressive
  if (analyse.statut === 'clair') return payload;
  if (analyse.statut === 'tronque') throw new Error('Format chiffré invalide');

  const decipher = crypto.createDecipheriv(ALGO, getKey(), analyse.iv);
  decipher.setAuthTag(analyse.authTag);
  const plaintext = Buffer.concat([decipher.update(analyse.ciphertext), decipher.final()]);
 ...
 *
 * Le contrôle porte sur la forme complète (alphabet base64, quatre segments,
 * IV de 12 octets, tag de 16), et non sur le seul préfixe : « v1:n-importe-quoi »
 * n'est pas un chiffré et ne doit pas être présenté comme tel à une migration.
 *
 * @param {unknown} value
 * @returns {boolean}
 */
export function isEncrypted(value) {
  return typeof value === 'string' && analyserPayload(value).statut === 'chiffre';
}
