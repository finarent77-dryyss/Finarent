...
export function mailingToText(html) {
  return html
    ...
    // Les retours à la ligne du source HTML ne sont que de l'indentation :
    // les sauts du rendu texte viennent uniquement des balises de bloc.
    .replace(/[\r\n]+/g, ' ')
    .replace(/<a[^>]+href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi, (_, href, label) => {
      const texte = label.replace(/<[^>]+>/g, '').trim();
      // Lien sans libellé = image cliquable (le logo) : sans intérêt en texte.
      if (!texte) return '';
    ...
    // Les cellules d'une même ligne (puce + texte, label + valeur) doivent
