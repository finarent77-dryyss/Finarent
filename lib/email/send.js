...
import { envoiReelAutorise, simulerEnvoi, raisonGarde } from './garde-envoi.js';
...
/**
 * Adresse d'expédition du canal SMTP.
 *
 * `expediteur()` fait passer l'adresse d'expédition Brevo avant `SMTP_FROM` :
 * c'est juste pour l'API Brevo, qui envoie au nom de tout expéditeur vérifié.
 * Mais un relais SMTP authentifié — Hostinger, OVH, un compte de messagerie —
 * n'accepte d'envoyer qu'au nom de la boîte connectée. Avec `SMTP_USER` sur
 * `contact@finarent.fr` et un expéditeur `ne-pas-repondre@finarent.com`, le
 * relais refusait l'envoi : le repli SMTP, censé rattraper un échec Brevo,
 * échouait à son tour.
 *
 * Le canal SMTP utilise donc sa propre identité : `SMTP_FROM`, à défaut
 * `SMTP_USER`. Le nom affiché est conservé.
 *
 * @param {{ name?: string, email?: string } | null | undefined} from
 * @param {Record<string, string | undefined>} [env]
 */
export function expediteurSmtp(from, env = process.env) {
  const email = env.SMTP_FROM?.trim() || env.SMTP_USER?.trim() || from?.email;
  return { name: from?.name, email };
}
...
async function envoyerViaSmtp({ to, subject, html, text, from, replyTo, headers, pieces }) {
  ...
  const expediteurReel = expediteurSmtp(from);
  const info = await trans.sendMail({
    from: `"${expediteurReel.name}" <${expediteurReel.email}>`,
    to,
...
/**
 * Écrit une entrée dans `EmailLog` sans jamais lever.
 * Rend l'entrée créée, ou `null` si la base n'a pas répondu.
 */
async function tracer(donnees) {
  try {
    const p = await db();
    return await p.emailLog.create({ data: donnees });
  } catch (e) {
...
export async function sendMail({
  ...
  // ─── Garde d'envoi hors production (RUN-05) ─────────────────
  // Un `npm run dev` avec le relais Brevo configuré expédiait de vrais
  // messages, dont un vers la boîte d'exploitation `admin@finarent.com`.
  // Hors production, on rend le message au lieu de l'expédier : console +
  // dépôt dans `.email-preview/envois-simules/` (ignoré par git). Tout le
  // reste du parcours est identique — validation d'adresse, choix du gabarit,
  // en-têtes de désabonnement, journalisation dans `EmailLog` — pour que le
  // développeur observe le même comportement qu'en production, moins la
  // livraison. Détails et échappatoire : `lib/email/garde-envoi.js`.
  //
  // En production `envoiReelAutorise()` rend `true` : cette branche est
  // ignorée et la séquence Brevo → repli SMTP ci-dessous est inchangée.
  if (!envoiReelAutorise()) {
    const { messageId, fichiers } = await simulerEnvoi({ ...commun, commercial });
    // `status: 'SENT'` volontairement, et non `FAILED` : l'envoi n'a pas
    // échoué, il a été retenu. Marquer l'entrée en échec ferait apparaître de
    // fausses alertes dans l'admin et pousserait les appelants qui testent
    // `result.sent` (parrainage, invitations affiliés) sur leur branche
    // d'erreur — le développement divergerait de la production. Les marqueurs
    // `provider: 'CONSOLE'` et `simule: true`, ainsi que le messageId en
    // `@finarent.local`, rendent l'entrée reconnaissable sans ambiguïté.
    const entree = await tracer({
      ...champsLog,
      status: 'SENT',
    ...
    return { sent: true, provider: 'CONSOLE', messageId, simule: true, emailLogId: entree?.id };
  }
