...
export function nextBusinessDay(d = new Date()) {
  ...
  // Cette date alimente <ReqdExctnDt> du lot SEPA : une date passée ou un
  // week-end fait rejeter ou différer le virement par la banque.
  const annee = next.getFullYear();
  const mois = String(next.getMonth() + 1).padStart(2, '0');
  const jour = String(next.getDate()).padStart(2, '0');
