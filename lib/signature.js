 ...
 *
 * Décision du 10 septembre 2026 : Finarent reste sur cette signature interne,
 * et le module Yousign — jamais branché nulle part — a été supprimé. Cette voie
 * recueille et conserve elle-même les éléments de preuve. C'est
...
import { ipClient as ipClientDeConfiance } from './ip-client';
...
/**
 * Adresse IP du signataire, consignée dans la preuve de signature.
 *
 * Corrigé le 10 septembre 2026. L'implémentation précédente retenait la
 * PREMIÈRE entrée de « X-Forwarded-For ». Or cet en-tête est fourni par
 * l'appelant : le proxy ajoute la sienne à la fin, mais tout ce qui précède a
 * été écrit par le client. Un signataire pouvait donc choisir l'adresse
 * inscrite dans sa propre preuve de signature — de quoi la contester ensuite,
 * et de quoi en faire porter l'origine à un tiers.
 *
 * La détermination est déléguée au module partagé, qui lit l'entrée posée par
 * le mandataire de confiance. Le repli reste « inconnue » : une adresse
 * indéterminée est un fait consignable, une adresse fausse ne l'est pas.
 */
export function ipClient(request) {
  return ipClientDeConfiance(request);
}
