...
- `D-11` — **le client ne pouvait pas accepter son offre**. Plus large que l'absence d'un bouton : l'espace client n'affichait **aucune** offre, la page ne les chargeant même pas. La route `POST /api/offers/[id]/accept` existait, durcie, et n'était appelée par rien. **Décision : ajouter l'affichage et le bouton d'acceptation.** ✅ fait — un onglet « Offres » affiche montant, mensualité, taux, coût total, partenaire et échéance ; le bouton demande une confirmation rappelant montant et durée, et remonte tels quels les refus du serveur (« offre expirée » et « offre déjà acceptée » ne disent pas la même chose). Un brouillon reste invisible du client. Couvert par 5 tests sur la règle de visibilité — dont un qui vérifie qu'une offre au statut inconnu est **masquée** et non montrée, défaut trouvé par le test lui-même. **Non exercé de bout en bout** : le parcours réel exige une session client, il relève du guide de test manuel (partie Client).
