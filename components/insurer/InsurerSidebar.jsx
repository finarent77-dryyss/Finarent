        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.key}
              href={item.href}
              className={`flex-1 min-w-0 flex flex-col items-center gap-1 px-1 py-3 text-xs leading-tight text-center font-medium transition-all ${isActive ? 'text-accent' : 'text-gray-400'}`}
            >
